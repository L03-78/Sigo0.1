...
class MateriaViewModel(private val dao: MateriaDao) : ViewModel() {

    private val _materias = MutableStateFlow<List<Materia>>(emptyList())
    val materias: StateFlow<List<Materia>> = _materias

    fun cargarMateriasPorCuatrimestre(cuatrimestreId: Int) {
        viewModelScope.launch {
            _materias.value = dao.obtenerMaterias(cuatrimestreId)
        }
    }
}
